...
"""Trainable PyTorch Goal 3/4/5 Tasks.

These are the production-shaped Tasks for the first trainable baseline.  They
load a checkpoint once, preprocess one Study at a time, and return the frozen
Result dataclasses used by the existing writer and validator.
"""
...
from tasks.results import BinaryResult, CategoricalResult, Goal3Result, Goal4Result, Goal5Result
...
class TorchGoal3Task(_TorchTask, StudyTask[Goal3Result]):
    name = "goal3"
...
class TorchGoal4Task(_TorchTask, StudyTask[Goal4Result]):
    name = "goal4"
...
class TorchGoal5Task(_TorchTask, StudyTask[Goal5Result]):
    name = "goal5"
    ...
    def predict(self, context: PipelineContext) -> Goal5Result:
        ...
        core = _restore_mask(masks[0], t1ce.image.shape)
